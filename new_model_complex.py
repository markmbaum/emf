...
Ex, Ey = E_field(x_cond, y_cond, subconds, d_cond, d_bund, V_cond, p_cond, x, y)

E_x,E_y,E_prod,E_max = phasors_to_output(Ex,Ey)
...
with open('new_model_out_E.txt','w') as ofile:
    ofile.write('%5s\t%10s\t%10s\t%10s\t%10s\t%10s\t%10s\t%10s\t%10s\n' % ('x','Ex','Experc','Ey','Eyperc','Eprod','Eprodp','Emax','Emaxp'))
    for z in zip(x,E_x,Experc,E_y,Eyperc,E_prod,Eprodperc,E_max,Emaxperc):
        ofile.write('%5d\t%10.4f\t%10.4f\t%10.4f\t%10.4f\t%10.4f\t%10.4f\t%10.4f\t%10.4f\n' % z)


Bx, By = B_field(x_cond, y_cond, I_cond, p_cond, x, y)

B_x,B_y,B_prod,B_max = phasors_to_output(Bx,By)
