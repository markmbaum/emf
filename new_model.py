...
f_cond = np.array([60.])     #frequency (degrees)
x_cond = np.array([0.])    #x coordinate (feet)
y_cond = np.array([5.])    #y coordinate (feet)
subconds = np.array([1.])   #number of subconductors
d_cond = np.array([1.])   #diameter (inches)
d_bund = np.array([1.])
V_cond = np.array([500.])   #phase-phase voltage (kV)
I_cond = np.array([500.])   #phase current (amp)
p_cond = np.array([0.])    #phase angle (degrees)
