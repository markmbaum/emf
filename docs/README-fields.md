...
To facilitate comparison between `emf.fields` and ENVIRO, there is a submodule of `emf.fields` called `emf.fields.enviro` with several functions for reading ENVIRO input and output files. The documentation for `emf.fields.enviro` is [here](emf.fields.enviro.html).
