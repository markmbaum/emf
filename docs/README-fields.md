![overhead-lines](img/overhead-lines.jpg)
...
* use [coefficients of potential](https://en.wikipedia.org/wiki/Coefficients_of_potential) to compute electric charge on each line from its voltage, then compute electric field phasors at the desired points in space from the charges, assuming the Earth's surface is at zero voltage
...
Another program called ENVIRO likely performs the same calculations for electric fields, but slightly different ones for magnetic fields. ENVIRO is part of the same software suite as SUBCALC, after which `emf.subcalc` is designed. ENVIRO is quite similar to FIELDS and `emf.fields`. It computes fields along line perpendicular to groups of parallel conductors. It also appears that ENVIRO accounts for "earth return currents" and for "induced currents on shield wires." These factors affect the magnetic field and difference between ENVIRO and `emf.fields` is greater for magnetic fields. The small differences between the electric field resuls of `emf.fields` and ENVIRO might be attributable to how the programs handle conductor bundles with multiple subconductors. There is some ambiguity with respect to how bundle diameters should be calculated from subconductor diameters and spacing.

To facilitate comparison between `emf.fields` and ENVIRO, there is a submodule of `emf.fields` called `emf.fields.enviro` with several functions for reading ENVIRO input and output files. The documentation for `emf.fields.enviro` is [here](http://mbaum1122.github.io/emf/emf.fields.enviro.html).
...
The `emf.fields.run` function does all of that and only requires the path of an excel workbook of templates. Templates can also be loaded into `SectionBook` objects for more targeted output using the `emf.fields.load_template` function. A brief example showing how a template is used can be found [here](using-a-template.ipynb). Alternatively, cross section models can be built entirely in Python, as [this notebook](fields-workflow-from-scratch.ipynb) demonstrates in an explicit manner and [this other notebook](underground-line-optimization.ipynb) demonstrates with fewer comments.
