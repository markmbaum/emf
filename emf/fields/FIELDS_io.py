"""The FIELDS_io module provides a handful of functions for streamlining the
file management process required by the old modeling program called FIELDS.

There are functions for reading the DAT files generated by FIELDS and converting
them into standard csv files (targeting a single DAT file (convert_DAT) or all
DAT files in a given directory and all its subdirectories (convert_DAT_crawl,
which has an additional option to bundle all DAT files found in a given
directory inside single xlsx files for easy access.)).

There are also functions that write the data in existing CrossSection objects
to formatted text files with FLD extensions, to be used as input files with
FIELDS instead of using the program's troublesome and error-prone menus to
input data. These functions are to_FLD, to_FLDs, and to_FLDs_crawl. CrossSection
objects are generated by reading from a template excel file, using the
load_template function. Converting all the sheets in a template excel file to
FLD files is a one line process:
    fields.to_FLDs(template_file.xlsx)"""
