...
class Results(object):
    """Results objects store the results of magnetic field modeling generated by the SubCalc program or an emf.fields Model object. A Results object can be created by passing grid results directly or by passing a dictionary of grid results. The function subcalc.load_results is the best way to generate a Results object from a .REF file containing SubCalc results. The Results object can be saved to a more flexible (and smaller) excel file with Results.export(). Then the excel file can be read back into a Results object using subcalc.load_results().

    Results objects have a 'Bkey' property that determines which component of the magnetic field results is accessed by the Results.B property. For example, when Results.Bkey is set to 'Bmax' all operations involving the grid of magnetic field results accessed by Results.B will operate on the 'max' field component. When Bmax is set to 'Bz' all operations deal with the vertical 'z' component of the field, and so on. This includes plotting.
    ...
    There are also methods for selecting a subdomain of a Results object and for shifting its x,y coordinates. These are Results.zoom() and Results.rereference() respectively.
