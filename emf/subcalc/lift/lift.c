...
void lift (double *x, double *y, double *z,
        ...
        double *integral){
    /*This function does the heavy lifting needed to compute the magnetic field generated by a current carrying wire segment in 3d space, at a set of points in that 3d space. The wire runs from the point (xa, ya, za) to (xb, yb, zb).
    args:
        x - an array of sorted unique x coordinates in the desired sample grid
        y - an array of sorted unique y coordinates in the desired sample grid
        z - an array of sorted unique z coordinates in the desired sample grid
            (will typically )
        delta_x - the wire's change along the x dimension (xb - xa)
        delta_y - the wire's change along the y dimension (yb - ya)
        delta_z - the wire's change along the z dimension (zb - za)
    ...
    for (i = 0; i < Nx; i++) {
        //store the sample point's y coordinate
        xp = x[i];

        for (j = 0; j < Ny; j++) {
            //store the sample point's x coordinate
            yp = y[j];

            for (k = 0; k < Nz; k++) {
                //store the sample point's z coordinate
                zp = z[k];

                //basically hash the indices for a single index
                idx = k*Ny*Nx + j*Nx + i;
                ...
                fac_x[idx] = delta_y*(zp - za) + delta_z*(ya - yp);
                fac_y[idx] = delta_z*(xp - xa) + delta_x*(za - zp);
                fac_z[idx] = delta_x*(yp - ya) + delta_y*(xa - xp);

                //store the path integral value for this location
                integral[idx] = B_integral(xp, yp, zp,
