...
def B_field_grid(a, b, I, ph, x, y, z):
    """Calculate the magnetic field generated by a current carrying segment of wire at points in a fixed-height 2 dimensional grid
    args:
    ...
    lift.lift_grid(x.ctypes.data_as(c_double_ptr),
...
def B_field_general(a, b, I, ph, x, y, z):
    ...
    integral = np.empty((N,), dtype=np.double)
    c_double_ptr = ctypes.POINTER(ctypes.c_double)
    lift.lift_general(x.ctypes.data_as(c_double_ptr),
            y.ctypes.data_as(c_double_ptr),
            z.ctypes.data_as(c_double_ptr),
            ctypes.c_double(delta_x),
            ctypes.c_double(delta_y),
